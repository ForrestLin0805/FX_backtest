...
in 3 moving averages strategy. It generates random numbers and set them as moving averages
length, then calculates strategy ratios for each case. User can set manually
...
ma_array = np.zeros((simulations_num, 3))
ma_array.astype(int)
...
for i in range(simulations_num):
    ...
    ma_random = np.random.random_integers(8, 80, (1, 3))
    if ma_random[0][0] == ma_random[0][1]:
        ma_random[0][1] += 1
    elif ma_random[0][1] == ma_random[0][2]:
        ma_random[0][2] += 1
    elif ma_random[0][0] == ma_random[0][2]:
        ma_random[0][2] += 1

    ma_array[i:] = ma_random

    # Creating an instance object of MaStrategy class and pass set and generated parameters
    strategy = strategies.Ma3Strategy(market_data=data, interval=interval, ma_type=ma_type,
                                      s_period=ma_random[0][0], l_period=ma_random[0][1], exit_period=ma_random[0][2],
...
best_exit_period = int(ma_array[best_argument][2])

# Run full backtest with best arguments
fulltest = strategies.Ma3Strategy(market_data=data, interval=interval, ma_type=ma_type,
                                  s_period=best_s_period, l_period=best_l_period, exit_period=best_exit_period,
                                  start_hour=start_hour, end_hour=end_hour, plot_strategy=True)
